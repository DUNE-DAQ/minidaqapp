# test_multiple_runs.py - make 3 runs consecutively and check there is appropriate output
...
nanorc_command_list="boot init conf start 1 resume wait 10 stop start 2 resume wait 10 stop start 3 resume wait 10 stop scrap terminate".split()
...
def test_data_files(run_nanorc):
    # We did three runs, so there should be 3 output files
    assert len(run_nanorc.data_files)==3
